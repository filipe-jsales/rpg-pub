...
public class EnemyController : MonoBehaviour
{
    ...
    private void DetectCeiling()
    {
        ...
        {
            // Calcula a dist�ncia vertical at� o teto
            float distanceToCeiling = ceilingHit.point.y - transform.position.y;

            // Se estamos pr�ximos o suficiente do teto, o morcego para de subir e fica ocioso
            if (distanceToCeiling < 0.2f)
            {
                _rigidbody2d.velocity = Vector2.zero; // Para o movimento
                _actionController.UseRunAnimation(false); // Volta � anima��o ociosa
                Debug.Log("Bat is idling on the ceiling");
            }
            else
            {
                // Se ainda n�o alcan�ou o teto, move o morcego para cima
                _rigidbody2d.velocity = new Vector2(0f, Mathf.Abs(moveSpeed)); // Certifica que moveSpeed � positivo
                _actionController.UseRunAnimation(true); // Continua com a anima��o de voar
